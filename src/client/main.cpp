...
/* 接收线程
因为控制台程序，需要我们使用cin手动输入信息，如果你不输入就一直被cin阻塞。那么就会存在一个问题：如果你一直不输入，那么程序就一直阻塞
在cin上。这就相当不合理，因为你不输入，程序阻塞，相应的你也接收不到别人发过来的信息。所以接收程序是为了解决这个问题的
*/
...
// 聊天客户端程序实现，main线程用于发送消息，子线程用作接收线程
int main(int argc, char **argv)
{
    ...
    // 连接服务器成功，启动接收子线程
    // readTask是线程名，使用了C++11的方式创建线程；clientfd是传递给readTaskHandler函数的参数。
...
void doLoginResponse(json &responsejs)
{
    ...
    else // 登录成功
    {
        ...
        {
            // 初始化,如果不初始化，每次loginout再登录，会保留上一次login时的信息
...
void readTaskHandler(int clientfd)
{
    ...
    {
        char buffer[1024] = {0};
        int len = recv(clientfd, buffer, 1024, 0); // 此处会阻塞
        if (-1 == len || 0 == len)
        {
            close(clientfd);
            exit(-1);
...
void showCurrentUserData()
{
    ...
    std::cout << "current login user => id:" << g_currentUser.gettId() << "name:" << g_currentUser.getName() << "state:" << g_currentUser.getState() << std::endl;
    std::cout << "------------------------------friend list------------------------------" << std::endl;
    if (!g_currentUserFriendList.empty())
    {
        for (User &user : g_currentUserFriendList)
        {
            std::cout << user.gettId() << " " << user.getName() << " " << user.getState() << std::endl;
        }
    }
    ...
    if (!g_currentUserGroupList.empty())
    {
        for (Group &group : g_currentUserGroupList)
        {
            ...
            {
                std::cout << user.gettId() << " " << user.getName() << " " << user.getState() << " " << user.getRole() << std::endl;
            }
        }
    }
...
// 注册系统支持的客户端命令处理
// int类型参数，是让我们传入clientfd，即socket的fd
...
{
    int idx = str.find(":"); // 此时的str只剩下friendid:message
    if (-1 == idx)
    {
...
{
    int idx = str.find(":"); // 此时的str只剩下groupname:groupdesc
    if (-1 == idx)
    {
        std::cerr << "chat command invalid!" << std::endl;
        return;
    ...
    std::string groupname_buffer = str.substr(0, idx);
    std::string groupdesc_buffer = str.substr(idx + 1, str.size() - idx);
    json js;
    js["msgid"] = CREATE_GROUP_MSG;
    js["id"] = g_currentUser.gettId();
    js["groupname"] = groupname_buffer;
    js["groupdesc"] = groupdesc_buffer;
    std::string buffer = js.dump();
...
void loginout(int clientfd, std::string str)
{
    ...
    if (-1 == len)
    {
        std::cerr<<"send loginout message error ->"<<buffer<<std::endl;
    }
    else
    {
